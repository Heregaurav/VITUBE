...
function EditProfile() {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-12 max-w-4xl mx-auto bg-white min-h-screen">
      <div className="mb-8 sm:mb-12">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-slate-900 tracking-tighter flex items-center gap-2 sm:gap-4">
          <Settings size={24} className="text-blue-600 sm:h-8 sm:w-8 md:h-9 md:w-9" />
          Settings
        </h1>
        <p className="text-slate-500 text-xs sm:text-sm mt-2 font-medium">Manage your account preferences</p>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 sm:gap-2 mb-8 sm:mb-10 bg-slate-50 p-1 sm:p-1.5 rounded-2xl border border-slate-100 overflow-x-auto w-full">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-1 sm:gap-2 px-3 sm:px-6 py-2 sm:py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${
              activeTab === tab.id
      ...
      {/* Account Tab */}
      {activeTab === "account" && (
        <form onSubmit={handleAccountUpdate} className="space-y-4 sm:space-y-6 w-full">
          <div className="space-y-1 sm:space-y-2">
            <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Full Name</label>
            <input
              type="text" value={accountForm.fullname}
              onChange={(e) => setAccountForm({ ...accountForm, fullname: e.target.value })}
              className="w-full bg-slate-50 text-slate-900 rounded-2xl px-3 sm:px-5 py-2.5 sm:py-4 text-sm outline-none focus:ring-2 focus:ring-blue-100 border border-slate-200 focus:border-blue-400 transition-all font-bold"
            />
          </div>
          <div className="space-y-1 sm:space-y-2">
          ...
          <button
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white font-black py-3 sm:py-4 px-6 sm:px-10 rounded-2xl transition-all shadow-lg shadow-blue-600/20 active:scale-95 disabled:opacity-50 text-xs uppercase tracking-widest w-full sm:w-auto"
          >
      ...
      {/* Password Tab */}
      {activeTab === "password" && (
        <form onSubmit={handlePasswordChange} className="space-y-4 sm:space-y-6 w-full">
          <div className="space-y-1 sm:space-y-2">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col items-start gap-6 sm:flex-row sm:items-center">
              ...
              <div className="text-center sm:text-left">
                ...
                <p className="text-blue-600 text-xs mt-2 font-bold">Tap or hover to change</p>
              </div>
            </div>
          </div>
